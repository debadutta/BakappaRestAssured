...
public class TestPatchAPIRequest {
	...
	public void patchAPIRequest() {
		...
		try {
			...
			File file4=new File(FileNameConstants.PATCH_API_REQUEST_BODY);
			String patchAPIRequestBody=FileUtils.readFileToString(file4, "UTF-8");
...
//			String authToken = JsonPath.read(tokenResponse.body().asString(),"$.token");
			...
			Response putResponse = //RestAssured
				...
				//PATCH API - Partial update booking
				Response patchResponse = given()
					.contentType(ContentType.JSON)
					.body(patchAPIRequestBody)
					.baseUri(FileNameConstants.BASE_URI)
					.header("Cookie","token="+authToken).log().body()
				.when()
				...
				String checkinDate=patchResponse.path("bookingdates.checkin");
